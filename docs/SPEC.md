...
- Рантайм: **Cobalt** (форк LuaJ, семантика Lua 5.2), CC:Tweaked / CC:Restitched.
...
- **Единственные CUT (физика платформы, не выбор):**
  ...
  - Аппаратная изоляция памяти — одна куча VM. Смягчается: бюджеты памяти
    (`collectgarbage("count")` на границах кванта) + kill при превышении + песочницы `_ENV`.
